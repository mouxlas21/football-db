...
CREATE TABLE IF NOT EXISTS league (
  ...
  tier       INTEGER,
  country_id BIGINT NOT NULL REFERENCES country(country_id) ON DELETE RESTRICT
);
CREATE INDEX IF NOT EXISTS ix_league_country ON league(country_id);
...
CREATE TABLE team (
  ...
  name    TEXT NOT NULL,
  type    TEXT NOT NULL DEFAULT 'club',  -- 'club' | 'national' | 'youth'
  ...
  CONSTRAINT team_kind_ck CHECK (
    (club_id IS NOT NULL AND national_country_id IS NULL) OR
...
CREATE INDEX ix_match_round ON match(round_id);
CREATE INDEX ix_match_kickoff ON match(kickoff_utc);
CREATE INDEX ix_match_teams ON match(home_team_id, away_team_id);
...
CREATE INDEX ix_app_match_team ON appearance(match_id, team_id);
CREATE INDEX ix_app_match_player ON appearance(match_id, player_id);
...
CREATE INDEX ix_sub_match ON substitution(match_id);
...
CREATE INDEX ix_event_match ON event(match_id);
CREATE INDEX ix_event_match_player ON event(match_id, player_id);
